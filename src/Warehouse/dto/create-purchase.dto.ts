...
export class CreatePurchaseDto {
  ...
  @ApiProperty({ example: 230000000 }) @IsNotEmpty() @IsNumber() totalPrice: number;
  @ApiProperty({ enum: PurchasePaymentType }) @IsNotEmpty() @IsEnum(PurchasePaymentType) paymentType: PurchasePaymentType;
  ...
  @ApiProperty({ example: 'Budi Santoso' }) @IsNotEmpty() @IsString() buyerName: string;
  @ApiProperty({ example: '081234567890' }) @IsNotEmpty() @IsString() buyerPhone: string;
